...
const AddFriend = ({ isOpen, onClose, onRequestSent }) => {
  ...
  const handleSendRequest = async (targetUsername) => {
    ...
    try {
      const res = await friendAPI.sendRequest({ username: targetUsername });
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Results */}
            <div className="max-h-80 overflow-y-auto scrollbar-thin px-5 pb-5">
              {username.length < 2 ? (
                ...
                <div className="space-y-2">
                  {searchResults.map((user, index) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl bg-dark-100/50 hover:bg-dark-100 transition-colors group"
                    >
                      {/* Avatar */}
                      <div className="relative">
                        <img
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium bg-gradient-accent text-white rounded-lg
